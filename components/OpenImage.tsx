...
import images, { OpenImageRecord } from "@/data/images";
...
export default function OpenImage({ slug, index = 0, alt = "", priority }: Props) {
  const pool = images.filter((item: OpenImageRecord) => item.slug === slug);
  if (!pool.length) {
    return null;
  }
  ...
  const requiresAttribution = licenseName
    ? REQUIRE_ATTRIBUTION.has(licenseName.toLowerCase())
    : false;
    ...
    <small className="block text-xs text-slate-500 mt-2">
      {requiresAttribution ? (
        <>
          ...
          <Link
            href={image.attribution?.sourcePage ?? "#"}
            target="_blank"
            className="underline"
          >
            source
          ...
          <Link
            href={
              image.license
                ? `https://creativecommons.org/licenses/${image.license.replace("-", "/")}/`
                : "#"
            }
            target="_blank"
            className="underline"
